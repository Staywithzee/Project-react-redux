...
function validate(fields) {
  ...
  if (!fields.stock && fields.stock !== '0') errors.stock = 'Stock is required.';
...
function ProductForm({ initial, isEdit, productId }) {
  ...
  const handleSubmit = async e => {
    ...
    const payload = { ...fields, price: Number(fields.price), stock: Number(fields.stock) };
    if (isEdit) {
...
export default function ProductFormPage() {
  ...
  const { data: product, isLoading, isError, error } = useGetProductByIdQuery(id, { skip: !isEdit });
  ...
  return (
    <div className={styles.page}>
      ...
      <ProductForm
        key={id ?? 'new'}
        initial={isEdit && product ? toFields(product) : EMPTY}
        isEdit={isEdit}
        productId={id}
      />
    </div>
